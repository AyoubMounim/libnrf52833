...
void twim_write(Twim const* const self, uint8_t* dataPtr, uint32_t dataLen, uint32_t slaveAddr){
  twim_stop(self);
  twim_eventsReset(self);
...
void twim_read(Twim const* const self, uint8_t* bufferPtr, uint32_t bufferLen, uint32_t slaveAddr){
  twim_stop(self);
  twim_eventsReset(self);
...
void twim_writeAndRead(
  Twim const* const self,
  uint8_t* dataPtr,
  uint32_t dataLen,
  uint8_t* bufferPtr,
  uint32_t bufferLen,
  ...
  twim_stop(self);
  twim_eventsReset(self);
  twim_shortLastTxToStartRxEnable(self);
  twim_shortLastRxToStopEnable(self);
  twim_setAddress(self, slaveAddr);
  pTwimData[self->unit]->txdPtr = (uint32_t) dataPtr;
  pTwimData[self->unit]->txdMaxCnt = (uint32_t) dataLen;
  pTwimData[self->unit]->rxdPtr = (uint32_t) bufferPtr;
